...
public class UpdateCompensationRecord extends TestSuiteBase {
	...
	public void ToUpdateCompnRecords(String EmpName,String PensionExitNotice,String PensionExitDate) throws Throwable
	{
		count++;
		...
		ModifyCompenstionRecord(EmpName,PensionExitNotice,PensionExitDate);
	...
	public void ModifyCompenstionRecord(String EmpName,String PensionExitNotice,String PensionExitDate) throws Throwable
	{
		try
		{
		...
		try
		{
			...
			{
				RewardTab(EmpName,PensionExitNotice,PensionExitDate);
			}
	...
	public void RewardTab(String EmpName,String PensionExitNotice,String PensionExitDate)throws Throwable
	{
		try
		{
					...
					endSearchingCompnRecord:
						while(x.hasNext())
						{
							...
							if(existsElement(OR.getProperty("compensationTableLocator")))
							{
								...
								if(existsElement(OR.getProperty("pensionExitNotice")))
								{
									Select selectByValue = new Select(driver.findElement(By.xpath(OR.getProperty("pensionExitNotice"))));
									// This select by value needs to be called from OR.Properties
									selectByValue.selectByValue(PensionExitNotice);
								}

								if(existsElement(OR.getProperty("pensionExitDate")))
								{
	...
	public void keyDates(String pensionExitDate)throws Throwable
	{
		try
		{
			...
			if(existsElement(OR.getProperty("pensionExitDate")))
			{
				getObject("pensionExitDate").sendKeys("");
				...
				Thread.sleep(4000L);
				getObject("pensionExitDate").sendKeys(formattedDate);
	...
	{
		return Test_Util.getData(Payroll_AutoEnrolment_Employee_Cease_SuiteXls,"UpdateCompensationRecord");
	}
