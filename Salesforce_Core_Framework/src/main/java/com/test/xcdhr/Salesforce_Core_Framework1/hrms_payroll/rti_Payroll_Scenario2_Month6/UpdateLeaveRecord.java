...
public class UpdateLeaveRecord extends TestSuiteBase
{
					...
					for(WebElement trElement : rows)
					{
						...
						for(WebElement tdElement : td_collection)
						{
							...
							if(tdElement.getText()!=null && tdElement.getText().equalsIgnoreCase("End date"))
							{
								...
								try
								{
									if(existsElementchkFor1mts(OR.getProperty("MatrnityEndDate")))
									{
										getObject("MatrnityEndDate").sendKeys("");
										getObject("MatrnityEndDate").clear();
										Thread.sleep(2000);
										...
										Thread.sleep(4000L);
										getObject("MatrnityEndDate").sendKeys(formattedDate);
										System.out.println("");
										System.out.println("The Maternity end date was entered sucessfully");	
										Thread.sleep(2000);
									}
								}
								catch(Throwable t)
	...
	{
		processDesiredTaxYearInputExcelFile(TaxYear);
		return Test_Util.getData(Payroll_RecognitionScenarioTwo_SuiteXls,"UpdateLeaveRecord");
	}
