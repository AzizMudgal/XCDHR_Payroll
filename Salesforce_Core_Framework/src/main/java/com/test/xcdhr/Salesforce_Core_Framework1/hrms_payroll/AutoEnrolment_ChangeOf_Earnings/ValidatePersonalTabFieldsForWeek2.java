...
public class ValidatePersonalTabFieldsForWeek2 extends TestSuiteBase {
	...
	public void CheckTestSkip() throws Exception{
		if(! Test_Util.IsTestcaseRunMode(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName())){

			Skip=true;
			Test_Util.ReportDataSetResult(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, "first", Test_Util.GetRowNum(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName()),"Skipped");
			//Test_Util.ReportDataSetResult(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName(), count+2, "Skip");
			APP_LOGS.debug("skipping the testcase" +this.getClass().getSimpleName() +" as the runmode is set to 'no' ");// this message would display in logs
			throw new Exception("Testcase is being skipped" + this.getClass().getSimpleName()+ "as it's Runmode is set to 'NO'"); // this msg would display in Reports.
		}
		// Load the runmodes of the tests
		runmodes=Test_Util.getDataSetRunmodes(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName());
	}
	...
	public void ValidatePersonalAndCompnRecords(String EmployerName,String EmpName,String Payrolid,String Frquency,String WeekName,String ExcelInputSheet,String FirstReportNameInApplication,String TestResultExcelFilePath,String worksheetNo,String PayrollVeiw) throws Throwable
	{
		count++;
		...
		CreateCompenstionRecord(EmpName,TestResultExcelFilePath,firstCellOfBody);
	...
	public void CreateCompenstionRecord(String EmpName,String TestResultExcelFilePath,String firstCellOfBody ) throws Throwable
	{
		try
		{
		...
		try
		{
			...
			if(existsElement(OR.getProperty("rewardtabClk")))
				 ...
				 */
				ReadsExpectedData(EmpName,TestResultExcelFilePath,AutoEnrolmntdate,AutEnrolNotfnChkboxStatusValue,AutoEnrolNotifyAttahment,effectTo,Action,EffctFrom,pensionScheme,employrPension,empPension);
	 ...
	 * 
	 */
	public void ReadsExpectedData(String EmpName,String TestResultExcelFilePath,String AutoEnrolmntdate,String AutEnrolNotfnChkboxStatusValue,String AutoEnrolNotifyAttahment,String effectTo,String Action,String EffctFrom,String pensionScheme,String employrPension,String empPension) throws Throwable
	{
		...
		String Auto_enrolment_date = cellToString(row4.getCell(2));
		
		
		String Effective_to = cellToString(row6.getCell(2));
		String Actionn = cellToString(row7.getCell(2));
		
		
		String Effective_from = cellToString(row8.getCell(2));
		String Pension_scheme = cellToString(row9.getCell(2));
		String Employer_contribution = cellToString(row10.getCell(2));
		String Employee_Contribution = cellToString(row11.getCell(2));
		
		row4.createCell(5).setCellValue(AutoEnrolmntdate);
		row5.createCell(5).setCellValue(AutoEnrolNotifyAttahment);
		row5.createCell(2).setCellValue(AutoEnrolNotifyAttahment);
		String New_attachment=cellToString(row5.getCell(2));
		
		
		row6.createCell(5).setCellValue(effectTo);
		row7.createCell(5).setCellValue(Action);
		
		
		row8.createCell(5).setCellValue(EffctFrom);
		row9.createCell(5).setCellValue(pensionScheme);
		row10.createCell(5).setCellValue(employrPension);
		row11.createCell(5).setCellValue(empPension);
		...
		if(employeeUnderTest != null && employeeUnderTest.equalsIgnoreCase(EmpName))
		{
			...
			///
			if(Actionn.equalsIgnoreCase(Action))
			{
			...
			///
			if(Effective_from != null && Effective_from.equalsIgnoreCase(Action))
			{
					...
					while(x.hasNext())
					{
						...
						if(existsElement(OR.getProperty("compenElementrow")))
						{
							...
							if(rownumv==2)
							{
								...
								if(existsElement(OR.getProperty("secondCompnTableLocator")))
								{
									WebElement SecondCompnTablelocator = driver.findElement(By.xpath(OR.getProperty("secondCompnTableLocator")));
									WebTable Comptable = WebTable.getTable(SecondCompnTablelocator);
									EffctFrom= Comptable.getTBody().getRow(3).getCell(1).getText();
							...
							{
								compensationlink.click();
								System.out.println("compensation link got clicked to 2nd compensation record");
	...
	{
		return Test_Util.getData(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls,"ProcessPayrollForWeek2");
	}
	...
	public void ReportDataSetResult()
	{
		...
		{
			IsTestPass = false;
			Test_Util.ReportDataSetResult(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName(), count+2, "Fail");
		}
		else
		{
			Test_Util.ReportDataSetResult(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName(), count+2, "Pass");
		}
	...
	public void ReportTestResult()
	{
		if(IsTestPass)
		{
			...
			Test_Util.ReportDataSetResult(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, "first", Test_Util.GetRowNum(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName()),"Pass");
		}
		else
		{
			Test_Util.ReportDataSetResult(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, "first", Test_Util.GetRowNum(Payroll_AutoEnrolment_Employee_ChangeInEarnings_SuiteXls, this.getClass().getSimpleName()),"Fail");
		}	
		closeBrowser();
	}
