...
public class SmallEmployerRelief extends TestSuiteBase
{
			...
			if(existsWebElement(postsTable11))
			{
				...
				outerloop:
					for(WebElement trElement1 : rows11)
					{
						...
						{
							System.out.println("row # "+row_num+", col # "+col_num+ "text="+tdElement.getText());
							if(tdElement.getText()!=null && tdElement.getText().equalsIgnoreCase("DONT TOUCH AUTO DIRSAPP EMPLOYER"))
							{
								Thread.sleep(5000L);
					...
					for(WebElement trElement : rows)
					{
						...
						for(WebElement tdElement : td_collection)
						{
							...
							if(tdElement.getText()!=null && tdElement.getText().equalsIgnoreCase("Small employer relief"))
							{
								 ...
								 */
								WebElement clkchkbox = driver.findElement(By.xpath("//following-sibling::td[1]/input[@type='checkbox'][@tabindex='11']"));
	...
	{
		processDesiredTaxYearInputExcelFile(TaxYear);
		return Test_Util.getData(Payroll_Statutory_AdoptionPaternitypay_SuiteXls,"SmallEmployerRelief");
	}
